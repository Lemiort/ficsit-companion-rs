...
fn test_merger_and_splitter_rates_from_cpp_export() {
    ...
    assert_eq!(ins[0].as_ref().unwrap(), "10", "Merger input 0 should be 10");
    assert_eq!(ins[1].as_ref().unwrap(), "30", "Merger input 1 should be 30");
    assert_eq!(outs[0].as_ref().unwrap(), "40", "Merger output should be 40");
    ...
    assert_eq!(s_ins[0].as_ref().unwrap(), "51", "Splitter input should be 51");
    assert_eq!(s_outs[0].as_ref().unwrap(), "15", "Splitter output 0 should be 15");
    assert_eq!(s_outs[1].as_ref().unwrap(), "36", "Splitter output 1 should be 36");
    assert_eq!(s_outs[2].as_ref().unwrap(), "0", "Splitter output 2 should be 0");
    ...
    let (c_ins, c_outs) = app.get_node_pin_rates(concrete_node_id).expect("Failed to get concrete pin rates");
    let (rate_str, _building) = app.get_node_building_info(concrete_node_id).expect("Failed to get building info for Concrete node");
